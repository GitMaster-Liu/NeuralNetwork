...
# 给x，y留出占位符，以便未来填充数据
x = tf.placeholder("float", [None, 156])
y_ = tf.placeholder("float", [None, 12])
# 设置输入层的W和b
W = tf.Variable(tf.zeros([156, 12]))
b = tf.Variable(tf.zeros([12]))
# 计算输出，采用的函数是softmax（输入的时候是one hot编码）
y = tf.nn.softmax(tf.matmul(x, W) + b)

# 第一个卷积层，5x5的卷积核，输出向量是32维
w_conv1 = weight_variable([5, 5, 1, 44])
b_conv1 = bias_variable([44])

x_image = tf.reshape(x, [-1, 26, 6, 1])
...
# 全连接层的w和b
w_fc1 = weight_variable([11, 12])
b_fc1 = bias_variable([12])
# 此时输出的维数是256维
h_pool2_flat = tf.reshape(h_pool1, [-1, 11])
...
for file_num in range(1,13):
    ...
    ff = open('textdigit' + file_num.__str__() + '.data')
    rr = ff.readlines()
    x_test2 = []
    y_test2 = []
    for i in range(len(rr)):
        x_test2.append(list(map(float,rr[i].split())))
    #print(x_test2)
    x_test2 = np.array(x_test2)
    x_test2 = x_test2.reshape(1, 156)
    #print (x_test2)
    x_test2=x_test2[0]
